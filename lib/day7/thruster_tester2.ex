defmodule Day7.ThrusterTester2 do
  ...
  def find_max_signal(program) do
    for a <- 5..9,
      ...
      if Enum.uniq(settings) |> length == 5 do
        test_settings(programs, settings)
      end
    end
    |> Enum.filter(& &1)
  ...
  def test_settings(
    ...
    {ptr_a, program_a, [output_a]} =
      Day7.IntcodeInterpreter2.start_program(program_a, inputs_a, ptr_a)

    inputs_b = if iteration == 0, do: [b, output_a], else: [output_a]

    {ptr_b, program_b, [output_b]} =
      Day7.IntcodeInterpreter2.start_program(program_b, inputs_b, ptr_b)

    inputs_c = if iteration == 0, do: [c, output_b], else: [output_b]

    {ptr_c, program_c, [output_c]} =
      Day7.IntcodeInterpreter2.start_program(program_c, inputs_c, ptr_c)

    inputs_d = if iteration == 0, do: [d, output_c], else: [output_c]

    {ptr_d, program_d, [output_d]} =
      Day7.IntcodeInterpreter2.start_program(program_d, inputs_d, ptr_d)

    inputs_e = if iteration == 0, do: [e, output_d], else: [output_d]

    {ptr_e, program_e, [output_e]} =
      Day7.IntcodeInterpreter2.start_program(program_e, inputs_e, ptr_e)
